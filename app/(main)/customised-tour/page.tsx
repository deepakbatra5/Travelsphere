...
import { PaperAirplaneIcon, SparklesIcon, MapPinIcon, CurrencyRupeeIcon, CalendarDaysIcon } from '@heroicons/react/24/outline'
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-2 lg:gap-8">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="mt-6 text-lg text-slate-600 max-w-xl">
            Can't find exactly what you're looking for? Let our travel experts craft a personalized itinerary just for you. Tell us where you want to go, and we'll take care of the rest.
          </p>
        ...
        <div className="rounded-3xl bg-white p-6 shadow-xl shadow-slate-200/50 md:p-10 border border-slate-100">
          <h2 className="text-2xl font-bold text-slate-900 mb-6">Tell us your requirements</h2>
          ...
          {success ? (
            <div className="rounded-2xl bg-green-50 p-8 text-center border border-green-100">
              ...
              <p className="mt-2 text-green-700">Our travel experts will get back to you within 24 hours with a customized itinerary.</p>
              <button 
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                  ...
                  <input required type="text" name="name" value={formData.name} onChange={handleChange} className="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-orange-500 focus:bg-white focus:ring-1 focus:ring-orange-500 transition" placeholder="John Doe" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Phone Number <span className="text-red-500">*</span></label>
                  <input required type="tel" name="phone" value={formData.phone} onChange={handleChange} className="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-orange-500 focus:bg-white focus:ring-1 focus:ring-orange-500 transition" placeholder="+91 9876543210" />
                </div>
              </div>

              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Email Address</label>
                <input type="email" name="email" value={formData.email} onChange={handleChange} className="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-orange-500 focus:bg-white focus:ring-1 focus:ring-orange-500 transition" placeholder="john@example.com" />
              </div>

              <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Destination <span className="text-red-500">*</span></label>
              ...
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Special Requirements / Travel Style</label>
                <textarea rows={4} name="message" value={formData.message} onChange={handleChange} className="w-full rounded-xl border-slate-200 bg-slate-50 px-4 py-3 text-sm focus:border-orange-500 focus:bg-white focus:ring-1 focus:ring-orange-500 transition" placeholder="Tell us about the kind of hotels you prefer, activities you like, or any specific places you want to visit..."></textarea>
              </div>
